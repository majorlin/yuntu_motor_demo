...
#if (((1ULL * MOTOR_CFG_ETMR_CLOCK_HZ * MOTOR_CFG_DEADTIME_NS) /               \
      1000000000ULL) == 0ULL)
...
static void MotorHwYtm32_SelectAdc0ExternalTriggerFromTmu(void) {
  CIM->CTRL =
      (CIM->CTRL & ~CIM_CTRL_ADC0_TRIG_SEL_MASK) | CIM_CTRL_ADC0_TRIG_SEL(1U);
}
 ...
 *
 * Current sensing usually happens in the middle of the zero-vector (all low
 * sides ON).
 *
 ...
 */
static inline void MotorHwYtm32_WriteHighSidePwm(eTMR_Type *const etmrBase,
                                                 uint8_t channel, float duty) {
  const uint32_t halfPeriodTicks = MOTOR_CFG_PWM_HALF_PERIOD_TICKS;
  const float clampedDuty =
      (duty < 0.0f) ? 0.0f : ((duty > 1.0f) ? 1.0f : (1.0f - duty));
...
/**
 * @brief Complete initialization of the ADC unit for either hardware or
 * software trigger.
 *
 * @param hardwareTrigger    Set true to use eTMR/TMU, or false to use software
 * execution.
 * @param sequenceInterrupt  Enable sequence completion interrupt (usually ADC
 * IRQ).
 * @param overrunInterrupt   Enable overrun detection interrupt.
 */
static void MotorHwYtm32_ConfigAdc(bool hardwareTrigger, bool sequenceInterrupt,
                                   bool overrunInterrupt) {
  ...
  adcConfig.trigger =
      hardwareTrigger ? ADC_TRIGGER_HARDWARE : ADC_TRIGGER_SOFTWARE;
...
static void MotorHwYtm32_InitTmu(void) {
  ...
  const tmu_user_config_t config = {.numInOutMappingConfigs = 1U,
...
static void MotorHwYtm32_InitEtmr(void) {
  ...
  const uint8_t usedChannels[] = {
      MOTOR_HW_PWM_U_HIGH_CH, MOTOR_HW_PWM_U_LOW_CH,  MOTOR_HW_PWM_V_HIGH_CH,
      MOTOR_HW_PWM_V_LOW_CH,  MOTOR_HW_PWM_W_HIGH_CH, MOTOR_HW_PWM_W_LOW_CH};
  uint32_t index;
  ...
  for (index = 0U; index < (sizeof(usedChannels) / sizeof(usedChannels[0]));
       index++) {
  ...
  (void)eTMR_DRV_SetChnOutMask(MOTOR_HW_ETMR_INSTANCE,
                               MOTOR_CFG_USED_PWM_CHANNEL_MASK, 0U, true);
...
static void MotorHwYtm32_InitPtmr(void) {
  ...
  (void)pTMR_DRV_InitChannel(MOTOR_HW_PTMR_INSTANCE, MOTOR_HW_PTMR_CHANNEL,
                             &ptmrChannelConfig);
}
...
void MotorHwYtm32_StartSpeedLoopTimer(void) {
  pTMR_DRV_ClearInterruptFlagTimerChannels(MOTOR_HW_PTMR_INSTANCE,
                                           MOTOR_HW_PTMR_CHANNEL);
...
bool MotorHwYtm32_ReadSoftwareFrame(motor_adc_raw_frame_t *frame) {
  ...
  while ((timeout > 0U) &&
         ((s_motorHwAdcBase->STS & ADC_STS_EOSEQ_MASK) == 0U)) {
    timeout--;
  }
...
void MotorHwYtm32_SwitchAdcToCurrentSensing(void) {
  ...
  s_motorHwAdcBase->CHSEL[0] = MOTOR_HW_ADC_CURRENT_A_CH;
  s_motorHwAdcBase->CHSEL[1] = MOTOR_HW_ADC_CURRENT_B_CH;
  s_motorHwAdcBase->CHSEL[2] = MOTOR_HW_ADC_CURRENT_C_CH;
  s_motorHwAdcBase->CHSEL[3] = MOTOR_HW_ADC_VBUS_CH;
  uint32_t temp = s_motorHwAdcBase->CFG0;
  temp &= ~ADC_CFG0_SEQLEN_MASK;
  temp |= ADC_CFG0_SEQLEN(3U);
  s_motorHwAdcBase->CFG0 = temp;
