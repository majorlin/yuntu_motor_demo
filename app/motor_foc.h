...
typedef struct
{
    ...
    float observer_lambda_vs;         /**< Adaptive flux-linkage estimate (Vs).             */
 ...
 *
 * Runs the full sensorless FOC pipeline at the PWM rate:
 * Clarke, flux observer, PLL, Park, d-q current PI, inverse Park, SVM.
 *
 ...
 *
 * @param[in,out] state              Persistent FOC state (speed integrator).
 * @param[in]     targetSpeedRadS    Target electrical speed (rad/s).
 * @param[in]     measuredSpeedRadS  Measured electrical speed (rad/s).
 ...
 *
 * Both inputs are first wrapped to [0, 2*pi). Result lies in (-pi, pi].
 *
