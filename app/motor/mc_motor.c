...
static void MC_Motor_UpdateMeasurements(mc_motor_t *motor, const mc_adc_sample_t *sample)
{
    motor->phase_currents_abc.a =
        ((float)sample->current_a_raw - (float)motor->current_a_offset_raw) * motor->config.adc_scale.current_a_per_count;
    motor->phase_currents_abc.b =
        ((float)sample->current_b_raw - (float)motor->current_b_offset_raw) * motor->config.adc_scale.current_a_per_count;
    motor->phase_currents_abc.c = -motor->phase_currents_abc.a - motor->phase_currents_abc.b;
    motor->vbus_v = (float)sample->vbus_raw * motor->config.adc_scale.vbus_v_per_count;
    motor->temperature_c =
        ((float)sample->temp_raw * motor->config.adc_scale.temp_c_per_count) + motor->config.adc_scale.temp_c_offset;
...
static void MC_Motor_RunCalibration(mc_motor_t *motor, const mc_adc_sample_t *sample)
{
    ...
    if (motor->calibration_count >= motor->config.hardware.calibration_samples)
    {
...
static void MC_Motor_RunCurrentLoop(mc_motor_t *motor, float electrical_angle_rad)
{
    ...
    voltage_limit_v = motor->config.control.voltage_utilization * motor->vbus_v * MC_ONE_BY_SQRT3_F;
    ...
    motor->voltage_command_dq.d = MC_PI_Run(&motor->id_pi,
                                            ...
                                            0.0f,
                                            1.0f / motor->config.control.current_loop_hz);
    ...
    MC_Svpwm_Generate(&motor->voltage_command_ab,
                      motor->vbus_v,
                      motor->config.hardware.min_duty,
                      &svm_result);
...
void MC_Motor_Init(mc_motor_t *motor, const mc_hal_ops_t *hal, const mc_user_config_t *config)
{
    ...
    motor->current_a_offset_raw = config->adc_scale.adc_full_scale / 2U;
    motor->current_b_offset_raw = config->adc_scale.adc_full_scale / 2U;
    ...
    motor->last_slow_loop_ts_s = 0.001f;
    ...
    MC_PI_Init(&motor->id_pi, &config->control.id);
    MC_PI_Init(&motor->iq_pi, &config->control.iq);
    MC_PI_Init(&motor->speed_pi, &config->control.speed);
    MC_Ramp_Init(&motor->speed_ramp,
                 config->command.speed_ramp_rpm_per_s,
                 config->command.speed_ramp_rpm_per_s,
                 0.0f);
...
void MC_Motor_FastLoop(mc_motor_t *motor, const mc_adc_sample_t *sample)
{
    ...
    dt_s = 1.0f / motor->config.control.current_loop_hz;
    ...
    motor->speed_feedback_rpm =
        (motor->config.motor.pole_pairs > 0U) ?
        ((motor->observer.output.speed_rad_s * 60.0f) / (MC_TWO_PI_F * (float)motor->config.motor.pole_pairs)) :
        0.0f;
    ...
    if (motor->state == MC_STATE_STARTUP)
    {
        ...
        if (startup_output.observer_handover_ready)
        {
            ...
            motor->handover_blend += dt_s / motor->config.forced_drag.observer_blend_time_s;
    ...
    if (motor->state == MC_STATE_RUN)
    {
        ...
        motor->iq_reference_a = MC_Math_Clamp(motor->iq_reference_a,
                                              -motor->config.motor.max_phase_current_a,
                                              motor->config.motor.max_phase_current_a);
...
void MC_Motor_Background(mc_motor_t *motor)
{
    ...
    target_speed_rpm = motor->input_request.run_request ?
        ((float)motor->input_request.direction * motor->config.command.default_speed_rpm) :
        0.0f;
